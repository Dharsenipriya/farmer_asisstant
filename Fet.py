...
def load_and_preprocess_data(csv_path):
    ...
    required_cols = ['Nitrogen', 'Potassium', 'Phosphorous', 'PH', 'Soil', 'Crop', 'Fertilizer']
    for col in required_cols:
        if col not in df.columns:
            raise ValueError(f"Missing required column: {col}")

    X = df[['Nitrogen', 'Potassium', 'Phosphorous', 'PH', 'Soil', 'Crop']]
    y = df['Fertilizer']
    return X, y
...
def save_model(model, encoder, filename="fertilizer_classifier.pkl"):
    import pickle
...
def predict_fertilizer(model, encoder):
    try:
    ...
    print(f"Recommended Fertilizer: {predicted_label}")
...
def main():
    ...
    predict_fertilizer(model, label_encoder)
